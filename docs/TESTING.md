...
## Calendar iteration — 23 September 2026

The download browser acceptance test was first run against the old UI and failed at the missing calendar action. `tests/unit/calendar.test.ts` adds 12 distinct cases: both repeated-hour occurrences, spring DST and year-boundary elapsed durations, UTC creation timestamp, UTF-8 folding/escaping, invalid titles and metadata/configuration, plus local-time/off-hour disclosure. `tests/e2e/calendar.spec.ts` adds three journeys inspecting actual downloaded bytes, committed-plan behavior with a pending invalid clock draft, local title changes, keyboard and narrow-screen controls/engineering links.

`npm run check` passes 59 domain cases in each of UTC, America/Los_Angeles and Asia/Tokyo (177 executions, 59 distinct cases), strict TypeScript and production build. All 19 Chromium journeys pass on both development and a fresh production preview with the built CSP active. Screenshots include refreshed desktop/mobile and the actual second-occurrence calendar handoff in `calendar-export.png`. Tests validate the generated RFC subset and file contents; they do not establish compatibility with every calendar client or prove that a calendar import happened.

## Human-use and accessibility pass — 23 September 2026

- `npm ci` and `npm run check` pass with the unchanged lockfile, strict TypeScript and production build. **59 unit cases** pass under UTC, America/Los_Angeles and Asia/Tokyo (177 executions, 59 distinct cases).
- The full **23 Chromium journeys** pass, including four new accessibility journeys for focus on opening/closing editors, hours errors, city additions/removals, repeated-instant choice, calendar error association, first-Tab skip navigation without changing shared-plan state, forced colors/reduced motion and 320px layout with semantic text sizes doubled. The final journey verifies Cancel/Escape after a six-city shared-plan restoration disables an already-open add form's trigger, preserving its URL and exact repeated-hour instant.
- Initial browser failures established missing form focus. The new tests assert surviving targets after conditional buttons disappear, rather than only checking that the controls can be clicked. Existing DST, exact ICS file, stale form/share-state, slider stability and mobile regressions remain green.
- Desktop/mobile captures were refreshed. [The work-hours editor](screenshots/accessible-hours.png) is a real 320px capture with doubled text and was visually inspected.

See [ACCESSIBILITY.md](ACCESSIBILITY.md) for the keyboard model, the hour band's equivalent field/slider controls, tested conditions and limits. External axe/production deployment verification belongs to the release owner; native screen-reader speech, physical touch and complete WCAG conformance are not inferred from these checks.
