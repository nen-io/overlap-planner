...
Each conditional editor now has an explicit focus lifecycle: reveal the first input, associate failed values with their error, then return to the trigger on completion/cancel. City additions focus the new heading and removals focus the stable Add action. Time ambiguity resolution returns to the time field. Skip links focus the existing control without navigating the hash, because that fragment is authoritative shared-plan data. Ref-based focus follows React commits and does not mutate the selected instant or unfinished drafts.
